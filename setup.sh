...
for f in bot.ini tokens.ini backup.key; do
    if [ ! -f "config/$f" ] && [ -f "$CONFIG_BACKUP/$f" ]; then
        cp "$CONFIG_BACKUP/$f" "config/$f"
        chmod 600 "config/$f" 2>/dev/null
        info "Restored config/$f from the backup at $CONFIG_BACKUP."
    fi
done
# backup.key decrypts every dump in backups/. Restoring it above is what keeps
# a rebuilt checkout from silently generating a NEW key and leaving every
# existing dump unreadable; warn loudly if dumps exist without one.
# (Each glob is tested on its own: `ls a* b*` exits non-zero as soon as EITHER
# pattern is unmatched, which would silence the warning on any host that has
# only one of the two dump kinds - i.e. on every host.)
if [ ! -f config/backup.key ] && { ls backups/yasuho-*.dump.gpg >/dev/null 2>&1 \
        || ls backups/yasuho-*.dump.enc >/dev/null 2>&1; }; then
    warn "Encrypted dumps exist in backups/ but config/backup.key is missing."
    warn "Restore that key from your out-of-band copy BEFORE the next start, or those dumps are unrecoverable."
fi
...
# backup.key is mirrored on its own: it is generated by the bot at runtime (not
# by this script) and is never a placeholder, so it must not sit behind the
# filled-in-bot.ini gate below. ./run.sh re-mirrors it on every start.
if [ -f config/backup.key ]; then
    mkdir -p "$CONFIG_BACKUP" && chmod 700 "$CONFIG_BACKUP" 2>/dev/null
    cp -f config/backup.key "$CONFIG_BACKUP/backup.key"
    chmod 600 "$CONFIG_BACKUP/backup.key" 2>/dev/null
fi
